{wezterm}: ''
  ...
  wezterm.on("format-tab-title", function(tab, tabs, _, conf)
    ...
    for _, t in ipairs(tabs) do
      if t.is_active then active_tab_index = t.tab_index end
    end
    ...
    local index_i = C.tabs.numerals == "roman"
      and roman_numerals[tab.tab_index + 1]
      or tab.tab_index + 1

    local index = string.format("%s%s%s ",
