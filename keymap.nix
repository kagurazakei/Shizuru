...
let
  yankScript = pkgs.writeShellScript "yazi-yank" ''
    for path in "$@"; do
      echo "file://$path" | ${pkgs.wl-clipboard}/bin/wl-copy -t text/uri-list
    done
  '';
      ...
      prepend_keymap = [
        {
          ...
          prepend_keymap = [
            ...
            {
              on = [ "<C-s>" ];
              run = "shell ${lib.escapeShellArg "$SHELL"} --block --confirm";
            ...
            { on = [ "T" ], run = "plugin --sync hide-preview", desc = "Hide or show the preview"; }
            {
              on = [ "y" ];
              run = [
                "shell ${yankScript} --confirm"
                "yank";
              ];
              desc = "yank files and copy to clipboard";
            }
            { on = "A", run = "create --dir", desc = "Create a dir"; }
            { on = [ "L" ], run = "plugin ouch --args=zip", desc = "Compress with ouch"; }
...
in {
  hj.files = {
    ".config/yazi/keymap.toml".source = pkgs.formats.toml { }.generate "yazi-keymap" keymap;
  };
}
